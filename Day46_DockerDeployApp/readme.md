...
c. Map php_host container's port 80 with host port 3004
...
curl <server-ip or hostname>:3004/

sudo docker images
sudo docker ps -a

# to check the database connectivity
http://localhost:3004/test-db-connect.php
```

```text
The Docker Compose sets up the infrastructure, but your PHP application code needs to actually use the connection parameters to connect to the database.
```